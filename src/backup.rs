...
// where a path is mirrored: relative to home, or under system/ relative to the system root
fn mirror(env: &Env, path: &Path) -> PathBuf {
    match path.strip_prefix(&env.home) {
        Ok(relative) => env.backup_dir().join(relative),
        Err(_) => env.backup_dir().join("system").join(path.strip_prefix(&env.sysroot).or_else(|_| path.strip_prefix("/")).unwrap_or(path)),
    }
}

// the first free backup path for a file: backups/x, then backups/x.1, backups/x.2, ...
pub fn next_target(env: &Env, path: &Path) -> PathBuf {
    let mirror = mirror(env, path);
    let taken = |candidate: &PathBuf| fs::symlink_metadata(candidate).is_ok();
    std::iter::once(mirror.clone())
...
pub fn backup(env: &Env, path: &Path) -> io::Result<PathBuf> {
    let target = next_target(env, path);
...
mod tests {
    ...
    fn paths_outside_home_mirror_under_system() {
        ...
        assert_eq!(mirror(&scratch, Path::new("/tmp/root/etc/greetd/config.toml")), scratch.backup_dir().join("system/etc/greetd/config.toml"));
    }
}
